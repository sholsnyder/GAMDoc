...
%The example shows the process of creating such a music from a gypsy-jazz version of the same Tetris music.
...
%\paragraph{Example: } Jardin, arbres, oiseaux, fontaine. A crow passes overhead and sticks around. To achieve realism, 3 elements are necessary : \begin{itemize}
...
%\begin{tikzpicture}
  %\begin{axis}[
    %xlabel=$x$,
    %ylabel={volume attenuation},
		%ymin=0, ymax=10,
		%minor y thick num=1
  %] 
    %\addplot[red, domain=0:10,samples=200
		%]{1/(x^2)}; 
  %\end{axis}
%\end{tikzpicture}
%
%
%\subsection{Routing, mixing and ducking}
%To visualize and adjust audio levels in any soundtrack, mixing tracks are irreplaceable. One track per audio element (An SFX switch, a musical instrument, a voice...) allows maximum flexibility. \\
%Tracks are grouped into sub-groups, which in turn are grouped together and routed to master tracks\footnote{N.B.: Unity doesn't distinguish tracks and groups, they are all called Audio Mixer Groups and act as both.}:\begin{itemize}
%\item Voices Master
%\item SFX Master
%\item Music Master
%\end{itemize}
